...
if (repoUrl) {
  ...
  const octokit = new Octokit({
    auth: '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',
  });

  octokit.repos.get({
    owner: library.value.repoOwner||'',
    repo: library.value.repoName||'',
  })
    .then(response => {
        library.value.nbStars = response.data.stargazers_count;
    })
    .catch(error => {
      console.error('Failed to fetch repository stars:', error);
    });
}
